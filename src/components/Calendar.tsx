...
export function Calendar() {
  ...
  const [newEvent, setNewEvent] = useState<{ date: string; title: string; type: "meeting" | "deadline" | "training" | "review" }>({ date: "", title: "", type: "meeting" });
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-md shadow-md w-96">
            ...
            <div className="mb-4">
              ...
              <select
                className="w-full border border-gray-300 dark:border-gray-700 rounded-md px-3 py-2"
                value={newEvent.type}
                onChange={(e) => setNewEvent({ ...newEvent, type: e.target.value as "meeting" | "deadline" | "training" | "review" })}
              >
            ...
            <div className="flex justify-end">
              <button
                className="px-4 py-2 bg-gray-300 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-md mr-2"
        ...
        <div className="border-t border-gray-200 dark:border-gray-700">
          <div className="grid grid-cols-7 gap-px bg-gray-200 dark:bg-gray-700">
            {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
              <div key={day} className="py-2 text-center text-sm font-medium text-gray-600 dark:text-gray-300">
                {day}
              </div>
            ))}
          </div>
          <div className="grid grid-cols-7 gap-px bg-gray-200 dark:bg-gray-700">
            {padding.map((_, index) => (
              <div key={`padding-${index}`} className="bg-gray-100 dark:bg-gray-800" />
            ))}
            {days.map((day) => (
              <div
                key={day}
